...
public class SDTCPReceiverv1 extends Agent {
	...
	public void recvSegment(Network net, Segment segment) {
		...
		case Keywords.Segments.Types.DATA:
			...
			ackSegment = new Segment(flow.getID(), Keywords.Segments.Types.ACK, ACKNum,
					Keywords.Segments.Sizes.ACKSegSize, this.srcHostID, this.dstHostID);
			sendSegment(net, ackSegment);
			break;
		case Keywords.Segments.Types.SYN:
			// SYNACK must be generated and sent to the sender
			ackSegment = new Segment(flow.getID(), Keywords.Segments.Types.SYNACK, ACKNum,
		...
		case Keywords.Segments.Types.FIN:
			Segment FINACKSegment = new Segment(flow.getID(), Keywords.Segments.Types.FINACK, segment.getSeqNum(),
					Keywords.Segments.Sizes.FINSegSize, this.srcHostID, this.dstHostID);
			sendSegment(net, FINACKSegment);
